...
public class score_view2 extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_score_view2);
        // Get the home button drawable
        getSupportActionBar().setTitle("Python User Progress");
        ...
        easyScoreRef = firebaseDatabase.getReference("Python_easy_scores");
        normalScoreRef = firebaseDatabase.getReference("Python_normal_scores");
        hardScoreRef = firebaseDatabase.getReference("Python_hard_scores");
        ...
        adapter = new StudentAdapter2(this, filteredStudentList);
        ...
        fab23.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(getApplicationContext(),System_analysis2.class);
                startActivity(intent);
                finish();
...
// Check the third item (index 2)
        ...
        easyScoreRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                updateScores(dataSnapshot, "Python_easy_scores");
            }
        ...
        normalScoreRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                updateScores(dataSnapshot, "Python_hard_scores");
            }
        ...
        hardScoreRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                updateScores(dataSnapshot, "Python_normal_scores");
            }
        ...
        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
            ...
            for (Student student : studentList) {
                if (student.getId().equals(studentId)) {
                    switch (scoreType) {
                        case "Python_easy_scores":
                            student.setEasyScore(score);
                            break;
                        case "Python_normal_scores":
                            student.setNormalScore(score);
                            break;
                        case "Python_hard_scores":
